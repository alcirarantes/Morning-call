...
export function buildHTML({ market, bcb, fred, analysis }) {
  ...
  // Helper: formata variacao com sinal
  const pct = (n) => {
    if (n == null) return '--';
    return (n >= 0 ? '+' : '') + n.toFixed(2) + '%';
  };

  // Helper: variacao calculada
  const calcPct = (val, prev) => {
